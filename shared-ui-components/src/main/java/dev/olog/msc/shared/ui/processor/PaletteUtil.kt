...
object PaletteUtil {
    ...
    fun ensureColors(background: Int, foreground: Int) : Triple<Int, Int, Int> {
        ...
        if (primaryTextColor == COLOR_INVALID
            ...
            val contrast = ColorUtil.calculateContrast(foreground, background)
            // We only respect the given colors if worst case Black or White still has
            // contrast
            val backgroundLight = backLum > textLum && ColorUtil.satisfiesTextContrast(background, Color.BLACK) || backLum <= textLum && !ColorUtil.satisfiesTextContrast(background, Color.WHITE)
            if (contrast < 4.5f) {
                if (backgroundLight) {
                    secondaryTextColor = ColorUtil.findContrastColor(
                            foreground,
                            background,
                            true /* findFG */,
                            4.5)
                    primaryTextColor = ColorUtil.changeColorLightness(
                            secondaryTextColor, -LIGHTNESS_TEXT_DIFFERENCE_LIGHT)
                } else {
                    secondaryTextColor = ColorUtil.findContrastColorAgainstDark(
                            foreground,
                            background,
                            true /* findFG */,
                            4.5)
                ...
                if (ColorUtil.calculateContrast(secondaryTextColor,
                                background) < 4.5f) {
