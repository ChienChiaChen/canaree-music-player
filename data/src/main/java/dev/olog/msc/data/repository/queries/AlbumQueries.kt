...
internal class AlbumQueries constructor(
    ...
    fun getRecentlyAddedAlbums(chunk: ChunkRequest?): Cursor {
        val query = """
            SELECT distinct $ALBUM_ID, $ARTIST_ID,
                $artistProjection as ${Columns.ARTIST},
                $albumProjection as ${Columns.ALBUM},
    ...
    fun getSiblingsChunk(albumId: Long, chunk: ChunkRequest?): Cursor {
        val artistQuery = """
             SELECT $ARTIST_ID
             FROM $EXTERNAL_CONTENT_URI
             WHERE $ALBUM_ID = ?
    ...
    fun getSongList(albumId: Long, chunk: ChunkRequest?): Cursor {
        val query = """
    ...
    fun countSongList(albumId: Long): Cursor {
        val query = """
            SELECT count(*)
            FROM $EXTERNAL_CONTENT_URI
            WHERE ${defaultSongSelection()} AND $ALBUM_ID = ?
