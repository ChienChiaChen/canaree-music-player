...
class PlaylistPodcastRepository @Inject constructor(
    ...
    override fun getAllAutoPlaylists(): Observable<List<PodcastPlaylist>> {
        return Observables.combineLatest(
                ...
                createAutoPlaylist(PodcastPlaylistGateway.PODCAST_LAST_ADDED_ID, autoPlaylistTitles[0], last),
                createAutoPlaylist(PodcastPlaylistGateway.PODCAST_FAVORITE_LIST_ID, autoPlaylistTitles[1], favorites),
                createAutoPlaylist(PodcastPlaylistGateway.PODCAST_HISTORY_LIST_ID, autoPlaylistTitles[2], history)
        )
        } }
    ...
    override fun observePodcastListByParam(param: Long): Observable<List<Podcast>> {
        return when (param){
            ...
            PodcastPlaylistGateway.PODCAST_HISTORY_LIST_ID -> historyDao.getAllPodcasts(podcastGateway.getAll().firstOrError())
    ...
    private fun removeFromAutoPlaylist(playlistId: Long, songId: Long): Completable {
        return when(playlistId){
            PodcastPlaylistGateway.PODCAST_FAVORITE_LIST_ID -> favoriteGateway.deleteSingle(FavoriteType.PODCAST, songId)
            PodcastPlaylistGateway.PODCAST_HISTORY_LIST_ID -> Completable.fromCallable { historyDao.deleteSinglePodcast(songId) }
