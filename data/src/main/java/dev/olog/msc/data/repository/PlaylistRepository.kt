...
class PlaylistRepository @Inject constructor(
    ...
    override fun getByParam(param: Long): Observable<Playlist> {
        val result = if (PlaylistGateway.isAutoPlaylist(param)){
            getAllAutoPlaylists()
        } else getAll()
    ...
    override fun getAllAutoPlaylists(): Observable<List<Playlist>> {
        return Observables.combineLatest(
                ...
                historyDao.getAllAsSongs(songGateway.getAll().firstOrError()).map { it.count() } // history
                        ...
                        createAutoPlaylist(PlaylistGateway.LAST_ADDED_ID, autoPlaylistTitles[0], last),
                        createAutoPlaylist(PlaylistGateway.FAVORITE_LIST_ID, autoPlaylistTitles[1], favorites),
                        createAutoPlaylist(PlaylistGateway.HISTORY_LIST_ID, autoPlaylistTitles[2], history)
                ) }
    }
    ...
    override fun observeSongListByParam(playlistId: Long): Observable<List<Song>> {
        return when (playlistId){
            ...
            PlaylistGateway.FAVORITE_LIST_ID -> favoriteGateway.getAll()
            PlaylistGateway.HISTORY_LIST_ID -> historyDao.getAllAsSongs(songGateway.getAll().firstOrError())
