...
class DetailFragmentPresenter @Inject constructor(
    ...
    fun removeFromPlaylist(item: DisplayableItem): Completable {
        ...
        if (playlistId == AutoPlaylistType.FAVORITE.id) {
    ...
    private fun handleMostPlayedHeader(
        list: MutableList<DisplayableItem>,
        isEnabled: Boolean
    ...
    private fun handleRecentlyAddedHeader(
        list: MutableList<DisplayableItem>,
        isEnabled: Boolean
    ): MutableList<DisplayableItem> {
        if (isEnabled && list.isNotEmpty()) {
    ...
    private fun handleRelatedArtistsHeader(
        list: MutableList<DisplayableItem>,
        isEnabled: Boolean
    ): MutableList<DisplayableItem> {
        if (isEnabled && list.isNotEmpty()) {
